...
void fill_player_tiles(UINT8 base_u, BOOL is_flip) {
  ...
  if (is_flip) {
    for (i = 0; i < PLAYER_SPRITE_SIZE / 2; ++i)
      set_sprite_tile(
          PLAYER_SPRITE_BASE + i,
          TILESET_MAY_AT(base_u + (PLAYER_SPRITE_SIZE / 2 - 1) - i, 0));
    for (i = 0; i < PLAYER_SPRITE_SIZE / 2; ++i)
      set_sprite_tile(
          PLAYER_SPRITE_BASE + (PLAYER_SPRITE_SIZE / 2) + i,
          TILESET_MAY_AT(base_u + (PLAYER_SPRITE_SIZE / 2 - 1) - i, 2));
  } else {
    for (i = 0; i < PLAYER_SPRITE_SIZE / 2; ++i)
      set_sprite_tile(PLAYER_SPRITE_BASE + i, TILESET_MAY_AT(base_u + i, 0));
    for (i = 0; i < PLAYER_SPRITE_SIZE / 2; ++i)
      set_sprite_tile(PLAYER_SPRITE_BASE + (PLAYER_SPRITE_SIZE / 2) + i,
                      TILESET_MAY_AT(base_u + i, 2));
  }
}
